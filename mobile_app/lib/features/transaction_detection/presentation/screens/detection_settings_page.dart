...
class DetectionSettingsPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DetectionSettingsPage> createState() => _DetectionSettingsPageState();
}
...
class _DetectionSettingsPageState extends ConsumerState<DetectionSettingsPage> {
  ...
  void _parseLive(String text) {
    ...
    final parsed = TransactionParser.parseMessage(body: text, sourceType: 'TEST');
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.security, color: AppColors.darkTeal, size: 24),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Text(
                        'Your SMS & notifications are processed 100% locally on this device. Full message text is never uploaded to the server.',
                        style: TextStyle(fontSize: 12, color: AppColors.darkTeal, height: 1.3),
                      ),
                    ),
                  ],
                ),
                  ...
                  child: Column(
                    children: [
                      SwitchListTile(
                        ...
                        title: const Text('SMS Transaction Detection', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                        subtitle: const Text('Scan incoming bank SMS messages for debit/credit alerts', style: TextStyle(fontSize: 12)),
                      ...
                      SwitchListTile(
                        contentPadding: EdgeInsets.zero,
                        title: const Text('Notification Detection', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                        subtitle: const Text('Capture transaction alerts from banking and payment apps', style: TextStyle(fontSize: 12)),
                        value: settings.notificationEnabled,
                        activeColor: AppColors.darkTeal,
                        onChanged: (val) {
                          _updateSettings(settings.copyWith(notificationEnabled: val));
                        },
                      ),
                      const Divider(),
                      SwitchListTile(
                        contentPadding: EdgeInsets.zero,
                        title: const Text('Require Confirmation', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                        subtitle: const Text('Review every detected transaction before adding to expenses/income', style: TextStyle(fontSize: 12)),
                        value: settings.confirmationRequired,
                        activeColor: AppColors.darkTeal,
                        onChanged: (val) {
                          _updateSettings(settings.copyWith(confirmationRequired: val));
                        },
                      ),
                    ],
                  ),
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'e.g. Commercial Bank Alert: Rs. 4,500.00 debited from A/C **4589 at Keells Super...',
                          ...
                          child: Column(
                            ...
                            children: [
                                ...
                                children: [
                                  Text(
                                    ...
                                    style: TextStyle(
                                      ...
                                      color: _liveParsedResult!.transactionType == 'DEBIT' ? Colors.red[700] : Colors.green[700],
                                    ),
                                  ),
                                  Container(
                              ...
                              if (_liveParsedResult!.merchant != null)
                                Text('• Merchant: ', style: const TextStyle(fontSize: 12)),
                              ...
                              if (_liveParsedResult!.reference != null)
                                Text('• Reference: ', style: const TextStyle(fontSize: 12)),
                            ],
                          ),
                        ),
                        const SizedBox(height: 12),
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            onPressed: () async {
                              ...
                              final res = await notifier.recordTransaction(_liveParsedResult!);
                              if (mounted && res != null) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Transaction added to pending queue!'), backgroundColor: AppColors.success),
                                );
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      return Chip(
                        ...
                        avatar: const Icon(Icons.account_balance, size: 14, color: AppColors.darkTeal),
                      );
                    }).toList(),
                  ),
                ),
